...
public class HashTable implements MapInterface<String, Integer> {
    ...
    private int sophisticatedHash(String k) {
    	//System.out.println("We are in the sophisticated hash");
        int hash1 = 0;
        int hash2 = 0;
        int len = k.length();
        // Iterate through each character of the key and calculate two separate hashes
        for (int i = 0; i < len; i++) {
            hash1 = 31 * hash1 + k.charAt(i);
            hash2 = 37 * hash2 + k.charAt(i);
        }
        int hashedcode = (hash1 + hash2) % TABLE_SIZE;
        // Combine the two hashes and ensure it falls within the table size
        if(hashedcode<0) {
        	return hashedcode + TABLE_SIZE;
        }
        return hashedcode;
